...
def test_primary_source_is_preferred_when_same_claim_is_supported():
    ...
    result = assess_news_corroboration(
        (
            ("webscraping", "official.acme.example", claim),
    ...
    assert result.status == "corroborated"
    assert result.primary_hosts == ("official.acme.example",)
    assert result.supporting_hosts == ("official.acme.example", "wire.example")
