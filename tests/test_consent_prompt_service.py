...
def test_pending_user_gets_prompt_and_recorded_with_legacy_two_arg_callable() -> None:
...
def test_pending_user_gets_prompt_with_markup_when_callable_supports_it() -> None:
    svc = ConsentPromptService()
    user = _user(status="pending")
    sent: list[tuple[int, str, dict[str, object]]] = []

    async def _send(chat_id: int, text: str, reply_markup: dict[str, object]) -> None:
