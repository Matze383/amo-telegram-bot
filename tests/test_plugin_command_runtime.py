...
def test_auto_image_provider_refusal_output_acknowledges_received_image(tmp_path) -> None:
    db_url = f"sqlite:///{tmp_path / 'plugin_runtime_auto_image_provider_refusal.db'}"
    init_db(db_url)
    executor, sent, replied, _, _ = _mk_executor(
        tmp_path,
        db_url,
        "unrelated_provider_refusal_demo",
        """
async def handle_command(context, host_api):
    await host_api.reply(context.chat_id, context.message_id, "should-not-run")
""",
    ...
    class _RefusalProvider:
        ...
        async def analyze_async(self, request):
            return type("_R", (), {"provider": "ollama", "summary": "Ich kann auf diesen Tweet nicht antworten, da er ein NSFW-Bild enthält. Tsubasa Ozora"})()
    ...
    with sf() as session:
        audits = session.scalars(select(ImageAnalyzeAuditEvent).order_by(ImageAnalyzeAuditEvent.id)).all()
    assert audits[-1].outcome == "provider_refusal"
